<x-layouts::auth :title="__('pages.auth.login.title')">
    <div class="kt-card max-w-[420px] w-full">
        <form action="{{ route('login.store') }}" class="kt-card-content flex flex-col gap-5 p-10" id="sign_in_form" method="POST">
            ...
            <div class="text-center mb-2.5">
                ...
                <div class="flex items-center justify-center font-medium">
                    <span class="text-sm text-secondary-foreground me-1.5">
                        {{ __('pages.auth.login.need_account') }}
                    </span>
            ...
            <x-ui.divider :label="__('pages.auth.login.or')" />

            {{-- Campo e-mail --}}
            <x-ui.form-field label="{{ __('pages.auth.login.email') }}" name="email">
                <x-ui.input
                    id="email"
                    name="email"
                    type="email"
            ...
            {{-- Campo senha --}}
            <x-ui.form-field label="{{ __('pages.auth.login.password') }}" name="password">
                <x-slot:actions>
            ...
            {{-- Remember me --}}
            <x-ui.checkbox name="remember" label="{{ __('pages.auth.login.remember_me') }}" :checked="old('remember')" />
