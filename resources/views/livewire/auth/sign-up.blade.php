    ...
    <form action="#" class="kt-card-content flex flex-col gap-5 p-10" id="sign_up_form" method="post">
        <div class="text-center mb-2.5">
            <h3 class="text-lg font-medium text-mono leading-none mb-2.5">
                {{ __('auth.sign_up.title') }}
            </h3>
            <div class="flex items-center justify-center">
                <span class="text-sm text-secondary-foreground me-1.5">
                    {{ __('auth.sign_up.have_account') }}
                </span>
                <a class="text-sm link" href="{{ route('auth.sign-in') }}">
                    {{ __('auth.sign_up.sign_in_link') }}
                </a>
            </div>
        </div>
        ...
        <div class="flex flex-col gap-1">
            <label class="kt-form-label text-mono">
                {{ __('auth.sign_up.email_label') }}
            </label>
            <input class="kt-input"
                   name="user_email"
                   placeholder="{{ __('auth.sign_up.email_placeholder') }}"
                   type="text"
                   value=""/>
        </div>
        <div class="flex flex-col gap-1">
            <label class="kt-form-label font-normal text-mono">
                {{ __('auth.sign_up.password_label') }}
            </label>
            <div class="kt-input" data-kt-toggle-password="true">
                <input name="user_password"
                       placeholder="{{ __('auth.sign_up.password_placeholder') }}"
                       type="password"
                       value=""/>
        ...
        <div class="flex flex-col gap-1">
            <label class="kt-form-label font-normal text-mono">
                {{ __('auth.sign_up.confirm_password_label') }}
            </label>
            <div class="kt-input" data-kt-toggle-password="true">
                <input name="user_password_confirmation"
                       placeholder="{{ __('auth.sign_up.confirm_password_placeholder') }}"
                       type="password"
                       value=""/>
                <button class="kt-btn kt-btn-sm kt-btn-ghost kt-btn-icon bg-transparent! -me-1.5"
                        data-kt-toggle-password-trigger="true"
